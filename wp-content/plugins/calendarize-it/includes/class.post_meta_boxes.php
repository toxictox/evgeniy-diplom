...
class post_meta_boxes {
	...
	function save_post($post_id, $post){		
		...
		if ( !wp_verify_nonce( @$_POST[$this->metabox_meta_fields.'-nonce'], $this->metabox_meta_fields.'-nonce' )) {
			return $post_id;
		}
	...
	function dbx_post_sidebar( $post ){
		...
		if($post->post_type==$this->post_type){
			...
			echo sprintf("<input type=\"hidden\" name=\"%s-nonce\" value=\"%s\" />",$this->metabox_meta_fields,wp_create_nonce( $this->metabox_meta_fields.'-nonce' ));		
		}	
	}
