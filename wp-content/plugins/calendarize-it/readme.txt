...
Tested up to: 4.9.5
Stable tag: 4.6.5.83180


== DESCRIPTION ==
Calendarize it! - The Most Flexible and Feature-rich Calendar Solution available for WordPress
...
We recommend that you use the Automatic Setup that you will see in the Dashboard after installing and activating Calendarize it! 
Don’t forget to enter your license key in the Options > License tab. You will find your Item Purchase Code (License key) under downloads in your Codecanyon.net account. Entering your license key will give you access to the downloads section where you can download 20+ free add-ons and purchase our premium paid add-ons.

You can easily rename the pages of the templates after using the Automatic Setup. If you are looking for a more specialized view than the basic calendar go to http://calendarize.it and check out the many options in VIEW EXMAPLES menu. You will find all the different attributes you can use with the default shortcode.
...
* Bug Fixed: When turning on the option to force https on all add-on resources in the Troubleshooting tab, also force apply it to tooltip Ajaxurl. Some sites are inconsistent in the protocol value returned by site_url().
...
Version 4.5.4.81563 - September 25, 2017
* Bug Fixed: When using the rch_upcoming_events shortcode (Upcoming Events Widget) instead of the rhc_static_upcoming_events shortcode several attributes are not passed to the shortcode renderer
...
* New Feature: Added Media Settings for 
...
* Update: The date/time picker in wp-admin reads the Calendar First Day from Options > Calendarize Shortcode tab (changed from WordPress settings)
* Update: Minify CSS

Version 4.5.2.80997 - August 4, 2017
* New Feature: Added support for inserting the Ratings & Reviews Box with Visual Composer and manually as shortcode [rhc_rar]
...
* Bug Fixed: Featured Image outputs PHP warnings in signe event page
...
Version 4.5.0.80530 - June 21, 2017
...
Version 4.4.6.79775 - May 20, 2017
...
* Bug Fixed: When using the Event Link to add-on in a recurring event, and the URL contains a hash, the hash is not properly placed in the URL
...
* Update: Change the Taxonomy filter behavior. When selecting a term in the Taxonomy the label automatically gets unselected. When all terms are unchecked the label gets selected
...
* Bug Fixed: Deselecting item in Taxonomy Filter drop down not working properly
